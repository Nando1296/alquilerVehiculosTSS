...
export default function ResultadosAnteriores(){
  ...
  const handleSeleccionarSimulacion = (simulacionId) => {
    if (simulacionesSeleccionadas.length === 2) {
      ...
      if (index === -1) {
        setSimulacionesSeleccionadas([...simulacionesSeleccionadas, simulacionId]);
      } else {
                ...
                <tr key={simulacion.id}>
                  ...
                  <td>
                    <div>
                      ...
                      <input
                        type="checkbox"
                        checked={simulacionesSeleccionadas.includes(simulacion.id)}
                        onChange={() => handleSeleccionarSimulacion(simulacion.id)}
                      />
                    </div>
                  </td>
                </tr>
